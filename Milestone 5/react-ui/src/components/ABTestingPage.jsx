...
const API_BASE = "https://yassinemtg-ab-testing.hf.space";
...
//   const [ignoreScore, setIgnoreScore] = useState(true); // checked by default
  ...
  const fetchResults = async () => {
    ...
    try {
      ...
      if (image) {
        ...
        // response = await axios.post("https://yassinemtg-ab-testing.hf.space/search_best_image", formData, {
        response = await axios.post("http://localhost:7860/search_best_image", formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
          timeout: 50000
        });
  
      } else {
        // response = await axios.get(`https://yassinemtg-ab-testing.hf.space/search_best?query=${encodeURIComponent(query)}`, { timeout: 50000 });
        response = await axios.get(`http://localhost:7860/search_best?query=${encodeURIComponent(query)}`, { timeout: 50000 });
      }
  
      // ✅ Correct parsing of the A/B API response which returns 'top_k_results'
      const filtered = response.data.top_k_results.map(item => ({
        ...item,
        //image_urls: "https://via.placeholder.com/150?text=Product+Image" // Placeholder image
      }));
  
      setResults(filtered.slice(0, allProducts ? filtered.length : topK));
      setPage(1);
      setSelectedRoute(response.data.route); // ✅ Show the selected model in UI
    } catch (error) {
  ...
  return (
    <div className="container">
      <div className="inner">
        <h1>🔍 Smart Product Search (A/B Testing)</h1>
        ...
        <p className="results-summary">
            {results.length > 0
            ? `${results.length} products matched ${selectedRoute ? `| Model used: ${selectedRoute}` : ""}`
