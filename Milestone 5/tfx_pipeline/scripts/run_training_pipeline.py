...
data_root = os.path.abspath("../Milestone 4_test/data/processed")  # folder where merged_queries.csv is stored
...
def create_pipeline():
    ...
    #         slicing_specs=[{"feature_keys": ["label"]}],
    #         metrics_specs=[
    #             {
    #                 "metrics": [
    ...
    pusher = Pusher(
        model=trainer.outputs['model'],
        # model_blessing=evaluator.outputs['blessing'],  # 🔥 Use blessing from Evaluator
        push_destination=PushDestination(
            filesystem=PushDestination.Filesystem(
                base_directory=os.path.join(_pipeline_root, 'pushed_model')
            )
        )
    )
